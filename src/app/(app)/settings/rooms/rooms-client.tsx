...
import { createRoomAction, updateRoomAction, archiveRoomAction } from "@/app/(app)/settings/rooms/actions";
...
export function RoomManagementClient({ rooms, campuses }: { rooms: Room[]; campuses: Campus[] }) {
  ...
  async function handleEdit(values: RoomFormValues) {
    if (!editing) return;
    const result = await updateRoomAction(editing.id, values);
    if (!result.success) {
      toast.error(result.error);
      return;
    }
  ...
  async function handleArchive() {
    if (!archiving) return;
    const result = await archiveRoomAction(archiving.id);
    if (!result.success) {
      toast.error(result.error);
      return;
    }
  ...
  const columns: Column<Room>[] = [
    ...
    { key: "campus", header: "Campus", accessor: (r) => campusById.get(r.campusId)?.name ?? "—", className: "text-on-surface-variant" },
  ...
  return (
    <div>
      ...
      <Modal open={addOpen} onOpenChange={setAddOpen} title="Add Room" className="max-w-[24rem]">
        <RoomForm onSubmit={handleAdd} onCancel={() => setAddOpen(false)} campuses={campuses} />
      </Modal>

      <Modal open={!!editing} onOpenChange={(open) => !open && setEditing(null)} title="Edit Room" className="max-w-[24rem]">
        {editing && <RoomForm initialValues={editing} onSubmit={handleEdit} onCancel={() => setEditing(null)} campuses={campuses} />}
      </Modal>
