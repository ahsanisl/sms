...
import { createSchoolWithOwnerAction, updateSchoolAction, archiveSchoolAction } from "@/app/(app)/settings/schools/actions";
...
export function SchoolsClient({ schools }: { schools: { school: School; stats: SchoolStats }[] }) {
  ...
  async function handleAdd(values: SchoolFormValues) {
    const result = await createSchoolWithOwnerAction(values);
    if (!result.success) {
      toast.error(result.error);
      return;
    }
    toast.success(`${values.name} was added, with ${values.ownerName} as School Owner.`);
    setAddOpen(false);
    router.refresh();
  }

  async function handleEdit(values: SchoolFormValues) {
    if (!editing) return;
    const { name, tagline, address, phone, email, logoEmoji, reportCardFooter, showSignatureLines } = values;
    const result = await updateSchoolAction(editing.id, { name, tagline, address, phone, email, logoEmoji, reportCardFooter, showSignatureLines });
    if (!result.success) {
      toast.error(result.error);
      return;
    }
  ...
  function handleArchive() {
    ...
    startTransition(async () => {
      const result = await archiveSchoolAction(archiving.id);
      if (!result.success) {
        toast.error(result.error);
        return;
      }
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                {!isArchived && (
                  <Button variant="secondary" size="sm" className="flex-1" disabled={isPending} onClick={() => setArchiving(school)}>
