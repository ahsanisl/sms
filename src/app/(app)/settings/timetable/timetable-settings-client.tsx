...
import { saveTimetableSettingsAction } from "@/app/(app)/settings/timetable/actions";
...
export function TimetableSettingsClient({
  workingDays: savedDays,
  periods: savedPeriods,
  breakAfterPeriod: savedBreak,
}: {
  ...
  const [days, setDays] = useState<TimetableDay[]>(savedDays);
  ...
  async function handleSave() {
    ...
    if (periods.some((p) => !p.startTime.trim() || !p.endTime.trim() || p.endTime === "—")) {
      toast.error("Every period needs a start and end time.");
      return;
    }
    setSaving(true);
    const result = await saveTimetableSettingsAction(
      days,
      periods.map((p) => ({ period: p.period, startTime: p.startTime, endTime: p.endTime })),
      breakAfter,
    );
    setSaving(false);
    if (!result.success) {
