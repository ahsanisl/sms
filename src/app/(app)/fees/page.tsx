...
import { useFeesStore, useStudents, usePermissions } from "@/lib/store/hooks";
import { useSession } from "@/lib/auth/session-context";
import { useCampusScope } from "@/lib/campus-scope";
import { GRADE_ORDER, CLASSES } from "@/lib/mock/reference-data";
...
export default function FeesPage() {
  ...
  const canManageStructure = !!user && !!routePermissions[user.role]?.feesStructure;

  const scopedStudentIds = new Set(
    (scopedCampusId ? students.filter((s) => s.campusId === scopedCampusId) : students).map((s) => s.id),
  );
  const scopedInvoices = invoices.filter((i) => scopedStudentIds.has(i.studentId));
  const currentMonth = scopedInvoices.filter((i) => i.month === "August 2026");

  const totalRevenue = scopedInvoices.reduce((s, i) => s + i.paidAmount, 0);
  ...
  const collectionByGrade = GRADE_ORDER.map((grade) => {
    const classIds = CLASSES.filter((c) => c.grade === grade).map((c) => c.id);
    const gradeStudentIds = new Set(students.filter((s) => classIds.includes(s.classId)).map((s) => s.id));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Collected This Month" value={formatCompactPKR(collectedThisMonth)} icon="payments" trend={{ direction: "up", label: "August 2026" }} />
        <StatCard label="Outstanding Balance" value={formatCompactPKR(outstanding)} icon="account_balance_wallet" trend={{ direction: "down", label: "August 2026" }} />
        ...
        <div className="bg-surface border border-outline-variant rounded-xl p-lg card-shadow">
          ...
          <div className="space-y-4">
            <div className="flex justify-between text-body-md">
              ...
              <span className="font-semibold text-on-surface">{currentMonth.length}</span>
            </div>
            <div className="flex justify-between text-body-md">
              <span className="text-on-surface-variant">Fully Paid</span>
              <span className="font-semibold text-emerald-600">{currentMonth.filter((i) => i.status === "paid").length}</span>
            </div>
            <div className="flex justify-between text-body-md">
              <span className="text-on-surface-variant">Unpaid</span>
              <span className="font-semibold text-error">{currentMonth.filter((i) => i.status === "unpaid").length}</span>
            </div>
            <div className="flex justify-between text-body-md">
                  ...
                  <tr key={p.id} className="hover:bg-surface-bright transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-on-surface">{student?.name ?? "Unknown"}</span>
                      </div>
                    </td>
