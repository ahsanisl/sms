...
import { applyConcessionAction, reversePaymentAction } from "@/app/(app)/fees/invoices/[id]/actions";
...
export function InvoiceDetailClient({
  ...
  invoice: { id: string; invoiceNo: string; studentId: string; month: string; issueDate: string; dueDate: string; totalAmount: number; paidAmount: number; status: InvoiceStatus; items: InvoiceItemRow[] };
  studentName: string;
  ...
  async function handleApplyDiscount(values: ConcessionFormValues) {
    setBusy(true);
    const result = await applyConcessionAction(invoice.id, invoice.studentId, {
      type: values.type,
      label: values.label,
  ...
  async function handleReverse() {
    ...
    setBusy(true);
    const result = await reversePaymentAction(invoice.id, reversing.id, reversing.amount, reversalReason, new Date().toISOString().slice(0, 10));
    setBusy(false);
    if (!result.success) {
      toast.error(result.error);
      return;
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm p-lg">
        <div className="flex flex-col md:flex-row justify-between gap-6 pb-lg border-b border-outline-variant">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-title-lg font-semibold text-on-surface">{schoolName}</h3>
        ...
        <div className="py-lg border-b border-outline-variant">
          ...
          <p className="text-body-lg font-semibold text-on-surface">{studentName}</p>
          <p className="text-body-md text-on-surface-variant">
            {studentClassLabel} · Roll {studentRollNumber} · Parent: {studentParentName}
          </p>
        </div>
          ...
          <div className="divide-y divide-outline-variant/20">
            {payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between px-lg py-4">
                <div>
                  <p className={`text-body-md font-medium ${p.wasReversed ? "text-on-surface-variant line-through" : "text-on-surface"}`}>
                    {formatPKR(p.amount)} · {p.method.replace("_", " ")}
                  </p>
                  <p className="text-label-sm text-on-surface-variant">
                    {formatDate(p.date)} · Received by {p.receivedByName}
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <Button variant="secondary" onClick={() => { setReversing(null); setReversalReason(""); }} disabled={busy}>
              Cancel
            </Button>
