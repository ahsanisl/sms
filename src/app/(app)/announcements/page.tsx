...
import { useAnnouncementsStore, usePermissions } from "@/lib/store/hooks";
...
export default function AnnouncementsPage() {
  const { announcements: allAnnouncements, deleteAnnouncement } = useAnnouncementsStore();
  const { user } = useSession();
  const { routePermissions } = usePermissions();
  const { scopedCampusId } = useCampusScope();
  const canManage = !!user && !!routePermissions[user.role]?.announcementsCreate;

  const announcements = scopedCampusId
    ? allAnnouncements.filter((a) => !a.campusId || a.campusId === scopedCampusId)
    : allAnnouncements;
  ...
  function handleDelete() {
    if (!toDelete) return;
    deleteAnnouncement(toDelete.id);
