...
import { useClasses, useStudents, usePermissions } from "@/lib/store/hooks";
import { GRADE_ORDER, campusName, teacherName, wingForGrade } from "@/lib/mock/reference-data";
...
export default function ClassesPage() {
  ...
  const campusFiltered = user?.role === "teacher"
    ? classes.filter((c) => c.campusId === user.campusId)
    : scopedCampusId
      ? classes.filter((c) => c.campusId === scopedCampusId)
      : classes;
  const scoped = campusFiltered.filter((c) => c.status === "active");

  const filtered = wing === "All Classes" ? scoped : scoped.filter((c) => wingForGrade(c.grade) === wing);

  const grades = GRADE_ORDER.filter((g) => filtered.some((c) => c.grade === g));

  function handleAdd(values: ClassFormValues) {
    addClass(values);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <SummaryCard icon="group" label="Total Students Enrolled" value={students.filter((s) => filtered.some((c) => c.id === s.classId)).length} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {grades.map((grade) => {
          ...
          const gradeStudents = students.filter((s) => sections.some((c) => c.id === s.classId));
          return (
              ...
              <div className="p-6 flex-1">
                ...
                <div className="grid grid-cols-2 gap-4 my-6">
                  ...
                  <div className="bg-surface-bright rounded-lg p-3 border border-outline-variant/50">
                    ...
                    <p className="text-title-lg font-semibold text-primary">{gradeStudents.length}</p>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  {sections.map((cls) => (
                    ...
                    >
                      <span className="text-on-surface">{cls.section} <span className="text-on-surface-variant text-xs">({campusName(cls.campusId)})</span></span>
                      <span className="text-on-surface-variant text-sm">
                        {students.filter((s) => s.classId === cls.id).length} Students • {teacherName(cls.classTeacherId)}
                      </span>
                    </Link>
                  ))}
                </div>
      ...
      <Modal open={addOpen} onOpenChange={setAddOpen} title="Add Class" className="max-w-[36rem]">
        <ClassForm onSubmit={handleAdd} onCancel={() => setAddOpen(false)} />
      </Modal>
    </div>
  );
}
