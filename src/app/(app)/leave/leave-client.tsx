...
import { requestLeaveAction, reviewLeaveRequestAction } from "@/app/(app)/leave/actions";
...
export function LeaveClient({ view, requests, isAllCampuses }: { view: "teacher" | "admin"; requests: LeaveRequestRow[]; isAllCampuses: boolean }) {
  ...
  async function handleRequest(values: LeaveRequestFormValues) {
  ...
  function handleApprove(request: LeaveRequestRow) {
    startTransition(async () => {
      const result = await reviewLeaveRequestAction(request.id, "approved");
      if (!result.success) {
        toast.error(result.error);
        return;
      }
  ...
  function handleReject() {
    if (!rejecting) return;
    startTransition(async () => {
      const result = await reviewLeaveRequestAction(rejecting!.id, "rejected", rejectNote);
      if (!result.success) {
        toast.error(result.error);
        return;
      }
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <StatCard label="Total Requests" value={String(requests.length)} icon="event_busy" />
        ...
        <div className="bg-surface-container-lowest rounded-lg border border-outline-variant/50 shadow-sm">
          ...
          <div className="divide-y divide-outline-variant/20">
            ...
            {requests.length === 0 && <EmptyState icon="event_busy" title="No leave requests yet" description="Requests you submit will show up here." />}
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <StatCard label="Approved" value={String(requests.filter((r) => r.status === "approved").length)} icon="task_alt" />
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-lg border border-outline-variant/50 shadow-sm">
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-outline-variant/20">
              {requests.map((r) => (
                <tr key={r.id} className="hover:bg-surface-bright transition-colors">
                  <td className="p-4 pl-lg font-medium text-on-surface">{r.teacherName}</td>
                  ...
                  <td className="p-4 pr-lg text-right">
                    {r.status === "pending" ? (
                      ...
                      <span className="text-label-sm text-on-surface-variant">by {r.reviewedByName ?? "Staff"}</span>
                    )}
                  </td>
                </tr>
              ))}
          ...
          {requests.length === 0 && <EmptyState icon="event_busy" title="No leave requests" description="Staff leave requests will appear here for review." />}
        </div>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-outline-variant/40">
            <Button variant="secondary" onClick={() => { setRejecting(null); setRejectNote(""); }} disabled={isPending}>
              Cancel
            </Button>
